...
def model_fn(features, labels, mode, params):
    ...
    def _add_regular_rnn_layers(convolved, lengths):
        ...
        cells_fw = [cell(params['num_nodes'])
                    for _ in range(params['num_layers'])]
        cells_bw = [cell(params['num_nodes'])
                    for _ in range(params['num_layers'])]
        ...
        #   output_fw will be a Tensor shaped: [batch_size, max_time, cell_fw.output]
        #   output_bw will be a Tensor shaped: [batch_size, max_time, cell_bw.output]
    ...
    def _add_rnn_layers(convolved, lengths):
        ...
        # outputs is [batch_size, L, N] where L is the maximal sequence length and N is
        # the number of nodes in the last layer.
        mask = tf.tile(
            tf.expand_dims(tf.sequence_mask(
                lengths, tf.shape(tf.concat(outputs, 2))[1]), 2),
            [1, 1, tf.shape(tf.concat(outputs, 2))[2]])
        zero_outside = tf.where(mask, tf.concat(
            outputs, 2), tf.zeros_like(tf.concat(outputs, 2)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="lstm",
        help="Cell type used for rnn layers: cudnn_lstm, lstm")
    parser.add_argument(
        "--batch_norm",
        type="bool",
        default="False",
